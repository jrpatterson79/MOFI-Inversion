...
# ── Helper Functions ──────────────────────────────────────────────────────────
make_fwd(test_list) = s -> rasmussen_phasor(test_list, s, model)
make_obj(fwd_func, y, R_inv) = s -> 0.5 * dot(y - fwd_func(s), R_inv * (y - fwd_func(s)))
...
for w in 1:num_cases
    ...
    # Define forward model and calculate true data
    fwd_func = make_fwd(test_list)
